...
export function Game() {
  ...
  const settingsOpen = Boolean(room.projection && settingsRoomId === room.projection.public.room_id);
  ...
  return (
    <GameShell
      ...
      settingsOpen={settingsOpen}
        onToggleSettings={() => setSettingsRoomId(settingsOpen ? null : room.projection?.public.room_id ?? null)}
      onLeave={handleLeave}
      onVoteKick={(targetId) => room.send({ kind: "room.vote_kick", target_id: targetId })}
    >
      {content}
    </GameShell>
  );
