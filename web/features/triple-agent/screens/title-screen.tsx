...
export function TitleScreen({ playerName, setPlayerName, roomCode, setRoomCode, joining, onStart, onJoin, onOpenJoin, onCancelJoin, error }: { playerName: string; setPlayerName: (value: string) => void; roomCode: string; setRoomCode: (value: string) => void; joining: boolean; onStart: () => void; onJoin: () => void; onOpenJoin: () => void; onCancelJoin: () => void; error?: string }) {
    ...
    <div className="ta-rise ta-screen ta-title-screen items-center text-center">
      ...
      <div className="ta-title-actions flex w-full flex-col gap-4 text-left">
        ...
        <div className="flex w-full flex-col gap-3">
          <InkButton variant="orange" onClick={onStart} disabled={!hasPlayerName}>Start a room</InkButton>
          <InkButton onClick={onOpenJoin}>Join a room</InkButton>
        </div>
      </div>
      {joining ? (
        <div className="ta-join-backdrop" role="presentation" onClick={onCancelJoin}>
          <div className="ta-paper ta-join-dialog" role="dialog" aria-modal="true" aria-labelledby="join-room-title" onClick={(event) => event.stopPropagation()}>
            ...
            <input id="join-room-code" ref={roomCodeRef} className="ta-text-input mt-1 w-full border-0 border-b-4 border-black bg-transparent px-1 py-2 text-2xl uppercase tracking-[0.18em]" value={roomCode} onChange={(event) => setRoomCode(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6))} onKeyDown={(event) => { if (event.key === "Enter" && roomCode.length === 6) onJoin(); if (event.key === "Escape") onCancelJoin(); }} placeholder="XZ04NW" autoComplete="off" maxLength={6} />
            <div className="mt-5 flex gap-3">
              <InkButton variant="orange" className="flex-1" onClick={onJoin} disabled={roomCode.length !== 6 || !hasPlayerName}>Join room</InkButton>
              <InkButton className="flex-1" onClick={onCancelJoin}>Cancel</InkButton>
            </div>
          </div>
        </div>
      ) : null}
