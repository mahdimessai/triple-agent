...
function websocketUrl(identity: RoomIdentity): string {
  ...
  const url = base
    ? new URL(base)
    : new URL(typeof window !== "undefined" && window.location?.origin ? window.location.origin : "http://localhost:8080");
  url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
  url.pathname = "/ws";
  url.search = new URLSearchParams({ join_code: identity.join_code }).toString();
  return url.toString();
}
...
export function connectRoom(identity: RoomIdentity, onEvent: (event: RoomSocketEvent) => void): RoomSocket {
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (message.type === "session.authenticated") { authenticated = true; emit({ type: "open" }); return; }
  ...
  return {
    ...
    isOpen() {
      return !disposed && !closed && authenticated && socket.readyState === WebSocket.OPEN;
    },
  };
}
