...
import type { OperationResult, RoomProjection } from "./protocol";

export type OperationStatus = "enabled" | "disabled-by-config" | "recovered-only";
export type OperationCategory = "information" | "choice" | "hidden" | "public" | "variant";
...
const anonymousTip = operations.find((operation) => operation.id === "OneRandom")!;

export const liveOperationIds: ReadonlySet<string> = new Set([
  "Grudge",
  "Infatuation",
  "Share",
  "Detector",
...
export const hiddenAgendaMemberIds: ReadonlySet<string> = new Set([
  "Strain",
  "Grudge",
  "Infatuation",
  "Flip",
...
export const packOperationIds: ReadonlySet<string> = new Set([
  "Swap",
  "Undercover",
  "InfoForTwo",
  "ChooseVoteShield",
...
export function countDeckOperations(enabledIds: Iterable<string>): number {
  let count = 0;
  let hiddenAgenda = false;
  for (const id of enabledIds) {
    if (id === "HiddenAgenda" || hiddenAgendaMemberIds.has(id)) {
      hiddenAgenda = true;
      continue;
    }
    count += 1;
...
export function getOperation(id: string | undefined): OperationDefinition {
  if (!id) return anonymousTip;
  return operations.find((operation) => operation.id === id) ?? anonymousTip;
}

export function operationIdForServerKind(kind: string | undefined): OperationId {
  if (kind === "Swap" || kind === "SpyTransfer") return "Swap";
  if (kind && operations.some((operation) => operation.id === kind)) return kind as OperationId;
  return "OneRandom";
}

export function operationBrief(operation: OperationDefinition): string {
  return operation.brief;
}

export function roomBriefing(operation: OperationDefinition, recipient: string, isRecipient: boolean, fallback: string): string {
  if (isRecipient) return operation.recipientBriefing || fallback;
