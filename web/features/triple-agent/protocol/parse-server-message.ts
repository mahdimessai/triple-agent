...
function isOperation(value: unknown): value is NonNullable<RoomProjection["public"]["operation"]> {
  ...
  if (value.acknowledged_player_ids !== undefined && !isStringArray(value.acknowledged_player_ids)) return false;
  return true;
}
...
function isLeaderboard(value: unknown): value is NonNullable<RoomProjection["public"]["leaderboard"]> {
  return Array.isArray(value) && value.every((entry) => {
    ...
    if (entry.initial_faction !== undefined && !isFaction(entry.initial_faction)) return false;
    if (entry.role !== undefined && !isString(entry.role)) return false;
    if (entry.defection !== undefined && !isString(entry.defection)) return false;
    if (entry.objective_kind !== undefined && !isString(entry.objective_kind)) return false;
    if (entry.objective_target !== undefined && !isString(entry.objective_target)) return false;
    if (entry.objective_name !== undefined && !isString(entry.objective_name)) return false;
...
function isPrivateProjection(value: unknown): value is RoomProjection["private"] {
  ...
  if (value.operation_acknowledged !== undefined && !isBoolean(value.operation_acknowledged)) return false;
  return true;
}

function isPublicVoteKick(value: unknown): value is NonNullable<RoomProjection["public"]["vote_kicks"]>[number] {
  return isRecord(value)
    && isString(value.target_id)
    && isString(value.target_name)
    && isNumber(value.votes)
    && isNumber(value.required)
    && isBoolean(value.has_voted);
}
...
function isRoomProjection(value: Record<string, unknown>): value is RoomProjection {
  ...
  if (room.vote_kicks !== undefined && (!Array.isArray(room.vote_kicks) || !room.vote_kicks.every(isPublicVoteKick))) return false;
  return true;
}
